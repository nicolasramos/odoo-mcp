...
- MCP `call_tool` expects arguments wrapped as `{ "payload": {...} }`.
- This file shows both raw tool payload examples and one explicit MCP envelope example.
"""
...
def wrap_for_mcp_call(tool_name: str, payload: Dict[str, Any]) -> Dict[str, Any]:
    """Build the canonical MCP `call_tool` request shape."""
    return {"name": tool_name, "arguments": {"payload": payload}}
...
def example_create_sale_order() -> Dict[str, Any]:
    ...
    return {
        ...
        "payload": {
            ...
            "lines": [
                {"product_id": 15, "product_uom_qty": 2.0, "price_unit": 99.99},
                {"product_id": 20, "product_uom_qty": 1.0, "price_unit": 149.50},
            ],
        },
    }
...
def example_get_partner_summary() -> Dict[str, Any]:
    ...
    """
    return {"tool": "odoo_get_partner_summary", "payload": {"partner_id": 42}}
...
def example_get_model_schema() -> Dict[str, Any]:
    ...
    """
    return {"tool": "odoo_get_model_schema", "payload": {"model": "sale.order"}}
...
def main() -> None:
    ...
    mcp_envelope = wrap_for_mcp_call("odoo_search", example_search_partners()["payload"])
    print(f"\n{'─' * 80}")
    print("MCP call_tool envelope example")
    print(f"{'─' * 80}")
    print(json.dumps(mcp_envelope, indent=2))
